...
ModelTriangle::ModelTriangle(const glm::vec3 &v0, const glm::vec3 &v1, const glm::vec3 &v2, Colour trigColour) :
		obj_name(), materialType(), textureKey(), hasTexture(), hasReflection(),hasRefraction(), lightReduction(), vertices({{v0, v1, v2}}), texturePoints(), colour(std::move(trigColour)), normal(), vertexNormals(), vertexLightings(), objectOrigin() {}
